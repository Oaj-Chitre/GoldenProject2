class Game {
  ...
  play(){
    ...
    if(player.distance > 4110 && passedFinish !== false){
      //gameState = 2;
    ...
    displayRank(){
      ...
      imageMode(CENTER);
      Player.getPlayerInfo();
      image(firstRankIMG, displayWidth/-4, -100+displayHeight/9, 200, 240);
      image(secondRankIMG, displayWidth/4, -100+displayHeight/9, 225, 270);
      
      textAlign(CENTER);
      textSize(50);
      for(var plr in allPlayers){
        if(allPlayers[plr].rank===1){
          text("1st :  " + allPlayers[plr].name, displayWidth/-4, displayHeight/10 + 76);
        }
        else if(allPlayers[plr].rank===2){
          text("2nd :  " + allPlayers[plr].name, displayWidth/4, displayHeight/10 + 76);
        }
